...
def test_is_sorted_on_sorted_integers():
    ...
    assert is_sorted([11]) is True  # Single item is trivially sorted
    assert is_sorted([23, 42]) is True  # Duplicate items are in order
    assert is_sorted([51, 67]) is True
    assert is_sorted([99, 100, 11]) is True
